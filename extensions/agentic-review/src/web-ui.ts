...
export class AgenticReviewWebUi {
	...
	private async handle(request: IncomingMessage, response: ServerResponse): Promise<void> {
		try {
			...
			if (method === "GET") {
				if (url.pathname === "/") {
					this.html(response, DASHBOARD_HTML);
					return;
				}
			...
			if (method === "POST") {
				if (!isSameOrigin(request)) {
					this.json(response, 403, { error: "Cross-origin settings request rejected" });
					return;
				}
...
function renderAnalyses(items){if(!items.length)return'';return'<section class="section"><h3>Bug analysis</h3><div class="grid">'+items.map(a=>'<div class="card"><div class="card-title"><span>'+esc(a.findingId)+'</span><span class="sev '+(a.disposition==='critical'?'bug':'nice-to-have')+'">'+esc(a.disposition)+'</span></div><div class="rationale">'+(a.isEdgeCase?'Edge case':'Normal path')+' · '+(a.impactsAcceptanceCriteria?'impacts acceptance criteria':'outside acceptance criteria')+'</div>'+(a.edgeCaseDefinition?'<div class="rationale" style="margin-top:7px">'+esc(a.edgeCaseDefinition)+'</div>':'')+'<div class="rationale" style="margin-top:7px">'+esc(a.reasoning)+'</div></div>').join('')+'</div></section>'}
...
function renderTimeline(events){return'<section class="section"><h3>Timeline</h3><div class="timeline">'+events.map(e=>'<div class="event '+esc(e.type)+'"><span class="event-dot"></span><span class="event-time">'+fmtTime(e.timestamp)+'</span><div><div class="event-msg">'+(e.stage?'<span class="event-stage">'+esc(e.stage)+'</span>':'')+esc(e.message)+'</div>'+(e.data&&Object.keys(e.data).length?'<details><summary>data</summary><pre>'+esc(JSON.stringify(e.data,null,2))+'</pre></details>':'')+'</div></div>').join('')+'</div></section>'}
function showView(view){const settings=view==='settings';document.getElementById('observer-view').classList.toggle('hidden',settings);document.getElementById('settings-view').classList.toggle('hidden',!settings);document.getElementById('nav-observer').classList.toggle('active',!settings);document.getElementById('nav-settings').classList.toggle('active',settings);if(settings){loadGitHubSettings();loadProviderSettings()}}
...
async function apiPost(path,body={}){const response=await fetch(path,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});const payload=await response.json();if(!response.ok||payload.error)throw new Error(payload.error||'Request failed');return payload}
...
async function bootstrap(){try{const [runData,statusData,githubData,providerData]=await Promise.all([fetch('/api/runs').then(r=>r.json()),fetch('/api/status').then(r=>r.json()),fetch('/api/settings/github').then(r=>r.json()),fetch('/api/settings/providers').then(r=>r.json())]);runs=runData.runs||[];watcher=statusData.watcher||watcher;github=githubData.connection||github;providers=providerData.providers||providers;if(runs[0])selectedId=runs[0].id;render();renderSettings();renderProviderSettings()}catch(e){console.error(e)}connect()}
function connect(){const c=document.getElementById('connection'),label=c.querySelector('span:last-child'),es=new EventSource('/api/events');const setLabel=()=>label.textContent='live · watcher '+(watcher.polling?'polling':watcher.waitingFor?'waiting: '+watcher.waitingFor:watcher.running?'on':'off');es.onopen=()=>{c.classList.add('live');setLabel()};es.onerror=()=>{c.classList.remove('live');label.textContent='reconnecting'};es.addEventListener('snapshot',e=>{const d=JSON.parse(e.data);runs=d.runs||runs;watcher=d.watcher||watcher;github=d.github||github;providers=d.providers||providers;if(!selectedId&&runs[0])selectedId=runs[0].id;setLabel();render();renderSettings();renderProviderSettings()});es.addEventListener('run',e=>upsert(JSON.parse(e.data)));es.addEventListener('watcher',e=>{watcher=JSON.parse(e.data);setLabel();render()});es.addEventListener('github',e=>{github=JSON.parse(e.data);renderSettings();if(github.connected)loadRepositories()});es.addEventListener('providers',e=>{providers=JSON.parse(e.data);renderProviderSettings()})}
document.getElementById('nav-observer').onclick=()=>showView('observer');document.getElementById('nav-settings').onclick=()=>showView('settings');document.getElementById('github-refresh-auth').onclick=refreshGitHubAuth;document.getElementById('repo-search').oninput=renderRepositoryOptions;document.getElementById('repo-search-clear').onclick=()=>{document.getElementById('repo-search').value='';renderRepositoryOptions();document.getElementById('repo-search').focus()};document.getElementById('repo-refresh').onclick=loadRepositories;document.getElementById('repo-save').onclick=saveRepository;document.getElementById('github-disconnect').onclick=disconnectGitHub;document.getElementById('anthropic-save').onclick=()=>saveProviderKey('anthropic');document.getElementById('anthropic-remove').onclick=()=>removeProviderKey('anthropic');document.getElementById('openai-save').onclick=()=>saveProviderKey('openai');document.getElementById('openai-remove').onclick=()=>removeProviderKey('openai');
