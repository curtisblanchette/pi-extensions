...
export interface RagDB {
  ...
  upsertFile(args: {
    ...
    size: number;
    format: string;
  }): { id: number; existing: FileRow | null };
...
export async function openDb(paths: RagPaths, dim: number, bitWidth = 4): Promise<RagDB> {
  ...
  // Validate or set dim.
  const storedDim = db
    .prepare<[], { value: string }>("SELECT value FROM meta WHERE key = 'dim'")
  ...
  return {
    ...
    insertChunks(args) {
      ...
      try {
        addVectors(ids, args.map((a) => a.embedding));
      } catch (err) {
    ...
    search(queryEmbedding, k, formatFilter) {
      ...
      if (formatFilter && formatFilter.length > 0) {
        ...
        allowlist = (db
          ...
          .all(...formatFilter) as Array<{ id: number }>).map((r) => r.id);
        if (allowlist.length === 0) return [];
      }

      const vectorHits = turbovec<TurboVecSearchResult[]>({
        cmd: "search",
        indexFile: paths.indexFile,
        dim,
    ...
    stats() {
      ...
      if (fs.existsSync(paths.indexFile)) {
        try {
          vectorCount = turbovec<{ count: number }>({
            cmd: "count",
            indexFile: paths.indexFile,
          }).count ?? null;
        } catch {
