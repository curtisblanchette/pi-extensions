...
export async function runConfigMenu(ctx: MenuCtx, paths: RagPaths): Promise<void> {
  ...
  while (true) {
    ...
    const picked = await ctx.ui.select(
      "RAG settings — pick a field to edit (ESC to exit)",
      choices,
    );
...
function labelFor(id: FieldId): string {
  switch (id) {
    ...
    case "chunkTokens": return "chunk tokens";
    case "chunkOverlap": return "chunk overlap";
    case "dim": return "embedding dim";
    case "bitWidth": return "turbovec bits";
    case "provider": return "provider";
    case "_reset": return "Reset";
    case "_exit": return "Done";
  }
}
...
async function editFolder(ctx: MenuCtx, paths: RagPaths, cfg: RagConfig): Promise<void> {
  const input = await ctx.ui.input(
    "Source folder to index (relative to project root)",
    cfg.folder,
  );
...
async function editFormats(ctx: MenuCtx, paths: RagPaths, cfg: RagConfig): Promise<void> {
  const input = await ctx.ui.input(
    "File formats (comma-separated, e.g. .md,.pdf,.txt,.docx)",
    cfg.formats.join(","),
  );
  if (input === undefined) return;
...
async function editExclude(ctx: MenuCtx, paths: RagPaths, cfg: RagConfig): Promise<void> {
  ...
  ctx.ui.notify(
    next.length > 0 ? `exclude = ${next.join(" ")}` : "exclude cleared.",
    "success",
  );
}
...
async function editModel(ctx: MenuCtx, paths: RagPaths, cfg: RagConfig): Promise<void> {
  ...
  if (candidates.length > 0) {
    ...
    const picked = await ctx.ui.select(
      `Pick embedding model (or "_custom_" to type one)`,
      choices,
    );
    if (picked === undefined) return;
    if (picked === "_custom_") {
    ...
    const custom = await ctx.ui.input(
      "Embedding model name (Ollama unreachable — typing blind)",
      cfg.model,
    );
    if (custom === undefined) return;
    next = custom.trim() || cfg.model;
  ...
  ctx.ui.notify(
    `model = ${next}. Dim cleared — run /rag-reset then /rag-index to rebuild.`,
    "warning",
  );
}
...
async function editBitWidth(ctx: MenuCtx, paths: RagPaths, cfg: RagConfig): Promise<void> {
  const picked = await ctx.ui.select(
    "turbovec bit width (2=smallest, 4=best recall)",
    ["2", "3", "4"],
  );
  if (picked === undefined) return;
  ...
  ctx.ui.notify(
    `bitWidth = ${bitWidth}. Run /rag-reset then /rag-index to rebuild vectors.`,
    "warning",
  );
}
...
async function editChunkTokens(ctx: MenuCtx, paths: RagPaths, cfg: RagConfig): Promise<void> {
  const input = await ctx.ui.input(
    "Approx chunk size in tokens (100–4000)",
    String(cfg.chunkTokens),
  );
  if (input === undefined) return;
