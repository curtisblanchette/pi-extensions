...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerCommand("rag-setup", {
    ...
    handler: async (_args, ctx) => {
      ...
      const runNow = await ctx.ui.confirm(
        "Index now?",
        `Run a first index of ${result.cfg.folder}/ ?`,
      );
      if (runNow) {
    ...
    handler: async (_args, ctx) => {
      ...
      if (!loadConfig(paths)) {
        const setup = await ctx.ui.confirm(
          "No RAG config",
          "This project has no RAG config yet. Run /rag-setup now?",
        );
        if (!setup) return;
        const result = await runSetup(
          ctx as unknown as Parameters<typeof runSetup>[0],
          paths,
          ctx.cwd,
        );
      ...
      await runConfigMenu(
        ctx as unknown as Parameters<typeof runConfigMenu>[0],
        paths,
      );
    },
  });
  ...
  pi.registerTool({
    ...
    promptSnippet:
      "Search the project's local document RAG index (turbovec + Ollama embeddings).",
    promptGuidelines: [
    ...
    async execute(_toolCallId, params, signal, _onUpdate, ctx) {
      ...
      if (!cfg.dim) {
        throw new Error("RAG config exists but no embedding dimension was detected. Run /rag-index to build the index.");
      }

      const k = Math.max(1, Math.min(30, params.k ?? 8));
      const formats = (params.formats ?? []).map((f) =>
  ...
  try {
    const result = await runIndex(cfg, paths, {
      ...
      onProgress: (p) => {
        ...
        ctx.ui.setStatus(
          STATUS_ID,
          `RAG ${p.scanned}/${p.total} · +${p.added} ~${p.updated} =${p.unchanged}${tail}`,
        );
      },
    });
    ctx.ui.setStatus(STATUS_ID, undefined);
