...
async function ensureIndexableConfig(
  paths: RagPaths,
  cfg: RagConfig,
  signal?: AbortSignal,
): Promise<RagConfig> {
...
server.registerTool(
  ...
  async ({ query, k = 8, formats = [], root }, extra) => {
    ...
    if (!cfg) {
      return textResult(`RAG not configured at ${paths.configFile}. Create it manually or call rag_config_write.`, true);
    }

    if (!cfg.dim) {
      return textResult("RAG config exists but dim is missing. Run rag_index first, or call rag_config_write with probeDim=true.", true);
    }
...
server.registerTool(
  ...
  async ({ root, force = false }, extra) => {
    ...
    if (!cfg) {
      return textResult(`RAG config missing at ${paths.configFile}. Create it manually or call rag_config_write first.`, true);
    }
