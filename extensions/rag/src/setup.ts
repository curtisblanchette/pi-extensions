...
export async function runSetup(
  ...
  if (!fs.existsSync(folderAbs)) {
    const create = await ctx.ui.confirm(
      ...
      `${folderAbs} does not exist. Create it?`,
    );
  ...
  // 2) Formats
  const formatsInput = await ctx.ui.input(
    "File formats to index (comma-separated)",
    base.formats.join(","),
  );
  ...
  const embeddingLikely = availableModels.filter((m) =>
    /embed|bge|nomic|mxbai|e5/i.test(m),
  );
  const candidates =
    embeddingLikely.length > 0
      ? embeddingLikely
  ...
  const pickedBitWidth = await ctx.ui.select(
    "turbovec bit width (2=smallest, 4=best recall)",
    [...new Set([String(base.bitWidth), "2", "3", "4"])] as string[],
  );
  ...
  const chunkTokensInput = await ctx.ui.input(
    "Approx chunk size in tokens",
    String(base.chunkTokens),
  );
  ...
  const overlapInput = await ctx.ui.input(
    "Chunk overlap in tokens",
    String(base.chunkOverlap),
  );
