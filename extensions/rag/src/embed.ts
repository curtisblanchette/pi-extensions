...
export async function listModels(cfg: RagConfig, signal?: AbortSignal): Promise<string[]> {
	...
	try {
		...
		throw new EmbedError(`Cannot reach Ollama at ${cfg.ollamaUrl}. Is it running? Try: ollama serve`, err);
	}
}
...
export async function embedBatch(cfg: RagConfig, inputs: string[], signal?: AbortSignal): Promise<number[][]> {
