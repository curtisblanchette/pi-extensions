...
export async function runIndex(cfg: RagConfig, paths: RagPaths, opts: IndexOptions = {}): Promise<IndexResult> {
	if (!cfg.dim) {
...
async function embedChunks(cfg: RagConfig, chunks: Chunk[], signal?: AbortSignal): Promise<number[][]> {
	const out: number[][] = [];
