...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("session_start", async (_event, ctx) => {
    try {
      ...
      ctx.ui.notify(
        `token-cost-tracker: failed to open DB: ${(err as Error).message}`,
        "error",
      );
    }
  });
  ...
  pi.on("message_end", async (event, ctx) => {
    ...
    try {
      ...
      ctx.ui.notify(
        `token-cost-tracker: appendEntry failed: ${(err as Error).message}`,
        "error",
      );
    }
    ...
    try {
      ...
      ctx.ui.notify(
        `token-cost-tracker: DB insert failed: ${(err as Error).message}`,
        "error",
      );
    }
  });
